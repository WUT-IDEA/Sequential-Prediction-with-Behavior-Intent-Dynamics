...
city = 'newyork'
dataset_name = "Crimes_2016.csv"
save_file_name = 'nyc16.txt'
hour_stage = 3 # 

geo_group = ['ADDR_PCT_CD', 'PREM_TYP_DESC']
item_group = ['Stage', 'KY_CD']
...
def preprocess():
    ...
    raw_data['Date'] = raw_data['CMPLNT_FR_DT'] + ' ' + raw_data['CMPLNT_FR_TM']
    raw_data['Time'] = raw_data['Date'].apply(lambda x: pd.to_datetime(x, format='%m/%d/%Y %H:%M:%S'))
    raw_data['Hour'] = raw_data['Time'].apply(lambda x: x.hour)
    raw_data['Stage'] = (raw_data['Hour'] / hour_stage).astype('int') + 1
    raw_data = raw_data.dropna(subset=['ADDR_PCT_CD']).astype({'ADDR_PCT_CD':'int'})
    
    # drop nan in 'Location_Description' except for 'DECEPTIVE PRACTICE'
    drop_index = raw_data.loc[(raw_data.PREM_TYP_DESC.isnull())].index
    raw_data = raw_data.drop(drop_index)
    raw_data['PREM_TYP_DESC'].fillna('empty',inplace=True)
